...
public class WaitToSeat3 extends Dialog {
    ...
    public WaitToSeat3(Context context) {
        super(context);

        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.activity_main_wait_issue3);
        ...
        //API버전 호환성 위함
        if(Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.O_MR1){ //API버전이 낮은 경우
        ...
        Button ok = (Button)findViewById(R.id.btnWaitOk3);
            ...
            public void onClick(View v) {
                ...
                if(hour == 0 && min == 0) {
                    ...
                    if((seatVariable.getEndDateTime().getHour() * 60 + seatVariable.getEndDateTime().getMinute()) //사용자 종료시간 > systemEndTime인 경우
                        ...
                        Toast.makeText(context, "금일 수면실 이용시간은 " + managerDB.printSystemEndTime() + "까지 입니다\n이용에 참고해주세요", Toast.LENGTH_SHORT).show();
                    }
                    new WaitToSeat4(context).show();
                }
                dismiss();
            }
        });
