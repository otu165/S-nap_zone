...
public class AdminOption3 extends Dialog {
    ...
    public AdminOption3(Context context) {
        super(context);
        requestWindowFeature(Window.FEATURE_NO_TITLE); //타이틀 바 제거
        setContentView(R.layout.activity_setting_option3); //레이아웃 설정
        ...
        String systemTime = managerDB.getSystemStartTime();
        ...
        Button ok = (Button)findViewById(R.id.btnSetOk3);
        Button close = (Button)findViewById(R.id.btnSetClose3);
        ...
        ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                int preHour = Integer.parseInt(managerDB.getSystemEndTime().substring(0,2));
                int preMinute = getSystemEndMinute();

                if(((preHour * 60 + preMinute) - (hour * 60 + min) < 180)) { //종료시간 - 시작시간 < 3인 경우 거절
                    Toast.makeText(context, "종료시간과 3시간 이상 차이나야 합니다", Toast.LENGTH_SHORT).show();
                    return;
                }

                //SystemDB 갱신
                managerDB.setSystemStartTime(hour, min * TIME_PICKER_INTERVAL);
                Toast.makeText(context, "시작 시간이\n " + managerDB.getSystemStartTime() + "으로 변경되었습니다", Toast.LENGTH_SHORT).show();

                //관리자 페이지의 텍스트뷰 갱신
                TextView tv = adminOption2.findViewById(R.id.setting1); // ***** 수정 필요함
    ...
    @SuppressLint("NewApi")
    private void setTimePickerInterval(TimePicker timePicker) { //'분' 15분 간격
